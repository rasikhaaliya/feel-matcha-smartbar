...
const MENU_ITEMS = [
  ...
  // --- BEST SELLERS (Smart Bar - Second Harvest - Customizable Strength) ---
  {
    id: 1,
  ...
  // --- PREMIUM SERIES (Manual Whisk - First Harvest - Customizable Strength) ---
  {
    id: 101,
    categoryId: 'premium',
...
const CUSTOMIZATIONS = {
  milk: [
    ...
    { id: 'arummi', name: 'Arummi (Cashew)', price: 8500 },
    { id: 'oatside', name: 'Oatside (Oat)', price: 10000 },
  ],
  sugar: [
  ...
  // Strength Logic: Smart Bar (Menu Biasa) vs Premium
  // [PERBAIKAN] Smart Bar sekarang bisa pilih Strong (+8k)
  smart_bar_strength: [
    { id: 'standard', name: 'Standard (2g)', price: 0, desc: 'Second Harvest' },
    { id: 'strong', name: 'Stronger (+1g)', price: 8000, desc: 'Second Harvest (+1g)' },
  ],
  // Premium tetap +10k
  premium_strength: [
    { id: 'standard', name: 'Standard (2g)', price: 0, desc: 'First Harvest' },
    { id: 'strong', name: 'Stronger (+1g)', price: 10000, desc: 'First Harvest (+1g)' }, 
  ],
  harvest: [
    { id: 'second', name: 'Second Harvest', price: 0, desc: 'Standard Grade' },
    { id: 'first', name: 'First Harvest (Umami)', price: 9000, desc: 'Rich & Intense' },
  ],
  // [PERBAIKAN] Add-ons: Matcha Shot dihapus
  addons: [
    { id: 'espresso', name: 'Espresso Shot', price: 8000 },
    { id: 'macchiato_cream', name: 'Macchiato Cream', price: 5500 },
    { id: 'cheese_cream', name: 'Cheese Cream', price: 5500 },
  ]
};
...
export default function FeelMatchaApp() {
  ...
  const addToCart = (item, options, note, quantity, selectedAddons = []) => {
    let extraPrice = 0;
    
    // Add options price safely
    if (options?.milk?.price) extraPrice += options.milk.price;
    if (options?.strength?.price) extraPrice += options.strength.price;
    
    // Jika Premium, ada biaya dasar grade (jika logic grade masih dipakai, tapi disini sudah include di harga base + strength)
    // Di sini kita hanya mengandalkan strength price yang berbeda untuk premium dan biasa
...
// ... LoginView, MenuView, UpsellModal, RnDOfferModal, FeedbackForm, SuccessView same as before ...
// ProductDetailView updated logic
...
function ProductDetailView({ item, onClose, onAddToCart }) {
  ...
  // Determine strength options based on category
  const strengthOptions = item.categoryId === 'premium' 
    ? CUSTOMIZATIONS.premium_strength 
    : CUSTOMIZATIONS.smart_bar_strength;

  const [options, setOptions] = useState({
    milk: CUSTOMIZATIONS.milk[0],
    sugar: CUSTOMIZATIONS.sugar[0],
    strength: strengthOptions[0], // Default strength
  ...
  let extraPrice = (options.milk?.price || 0) + (options.strength?.price || 0);
  ...
  const currentPrice = (item.price + extraPrice + addonTotal) * quantity;
    ...
    <div className="fixed inset-0 z-50 bg-white flex flex-col animate-in slide-in-from-bottom duration-300">
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-32 -mt-6 relative">
        ...
        {item.customizable && (
          <div className="space-y-6">
            ...
            {/* STRENGTH SELECTOR - NOW FOR ALL DRINKS */}
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                {item.method === 'smart_bar' && (
                   <span className="text-xs font-bold text-yellow-700 bg-yellow-100 px-2 py-1 rounded flex items-center">
                     <Lock size={10} className="mr-1"/> Smart Dosing
                   </span>
                )}
              </div>
            ...
            {/* ADD-ONS SECTION (Matcha Shot removed) */}
            <div>
              <div className="flex items-center space-x-2 mb-3">
                <Plus size={16} className="text-gray-400" />
                <label className="font-bold text-gray-900">Add-ons</label>
              </div>
              <div className="grid grid-cols-1 gap-2">
                {CUSTOMIZATIONS.addons.map(addon => (
                  ...
                  >
                    <div className="font-medium text-sm text-gray-700">{addon.name}</div>
                    <span className="text-xs font-bold text-green-700">+Rp {addon.price.toLocaleString()}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="absolute bottom-0 inset-x-0 bg-white border-t border-gray-100 p-4 pb-8 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <div className="flex items-center space-x-4">
          ...
          <button 
            onClick={() => onAddToCart(item, options, note, quantity, selectedAddons)}
                ...
                {item.customizable && (
                  <div className="text-xs text-gray-500 mt-1 space-y-0.5">
                    ...
                    <p className="font-semibold text-green-700">
                      Grade: {item.categoryId === 'premium' ? 'First Harvest' : 'Second Harvest'}
                    </p>
                    ...
                    {item.selectedAddons && item.selectedAddons.length > 0 && (
                      <p className="font-semibold text-gray-700">
                        + {item.selectedAddons.map(id => CUSTOMIZATIONS.addons.find(a => a.id === id).name).join(', ')}
                      </p>
                    )}
